...
export default function ResultsViewAvg({ status, results, onDownload, onStop, onReset, jobId }: ResultsViewAvgProps) {
    ...
    const openDownloadModal = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${API_BASE_URL}/fields-avg`);
                        ...
                        >
                            <div className="bg-white border-b border-slate-100 p-6 flex items-center justify-between rounded-t-[32px]">
                                <h3 className="font-bold text-slate-900 text-xl flex items-center gap-2">
                                    <Download size={22} className="text-blue-600" /> Export Averages Data
                                </h3>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="p-5 font-bold text-slate-500 uppercase text-xs tracking-wider">Profile Link</th>
                            <th className="p-5 font-bold text-slate-500 uppercase text-xs tracking-wider">Profile</th>
                            <th className="p-5 font-bold text-slate-500 uppercase text-xs tracking-wider text-center">Avg Metrics</th>
                            <th className="p-5 font-bold text-slate-500 uppercase text-xs tracking-wider text-center">Reels Parsed</th>
                                ...
                                <td className="p-5 min-w-[320px]">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <a href={result.link} target="_blank" className="font-bold text-slate-900 flex items-center gap-1.5 hover:text-blue-600 transition-colors text-sm break-all">
                                ...
                                <td className="p-5">
                                    <div className="flex items-center justify-center gap-4">
                                        <div className="flex flex-col items-center">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase">Avg Comments</span>
                                            <span className="font-bold text-slate-700">{result.avg_comments || 0}</span>
                                        </div>
                                        <div className="flex flex-col items-center border-l w-24 border-slate-100 pl-4">
                                            <span className="text-[10px] font-bold text-slate-400 uppercase">Avg Views</span>
                                            <span className="font-bold text-slate-700">{result.avg_videoplaycount || 0}</span>
                                        </div>
                                    </div>
                                </td>
                                <td className="p-5 text-center">
                                    <span className="font-bold text-slate-700 text-lg">{result.reels_scraped || 0}</span>
                                    <span className="text-slate-400 text-xs ml-1 font-semibold">reels</span>
                                </td>
                                <td className="p-5 text-center">
